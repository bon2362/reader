...
- [x] Task 1: Расширить `AnnotationRepositoryProtocol`
  - [x] 1.1 Добавить lookup для `Highlight`
  - [x] 1.2 Добавить lookup для `TextNote`
  - [x] 1.3 Добавить lookup для `PageNote`

- [x] Task 2: Реализовать lookup в `AnnotationRepository`
  - [x] 2.1 Реализовать поиск `Highlight` по `book_id` и `exchange_id`
  - [x] 2.2 Реализовать поиск `TextNote` по `book_id` и `exchange_id`
  - [x] 2.3 Реализовать поиск `PageNote` по `book_id` и `exchange_id`

- [x] Task 3: Проверить влияние на existing code paths
  - [x] 3.1 Убедиться, что новые методы не ломают существующие mock/stub-реализации
  - [x] 3.2 При необходимости обновить тестовые doubles

- [x] Task 4: Написать unit tests
  - [x] 4.1 Находится запись с совпадающим `exchangeId`
  - [x] 4.2 Не находится запись при другом `bookId`
  - [x] 4.3 Не находится запись при другом `exchangeId`
  - [x] 4.4 Legacy-записи с `NULL exchange_id` не вызывают ошибок
...
- Расширить protocol и concrete repository тремя lookup-методами по `bookId + exchangeId`
- Сохранить legacy-safe поведение для строк с `NULL exchange_id`
- Добавить unit tests для positive/negative/legacy cases по всем трём типам аннотаций

### Debug Log

- Проверены существующие usage sites `AnnotationRepositoryProtocol`; отдельных mock/stub conformers, требующих правки, не найдено
- Прогнан targeted test run: `xcodebuild test -only-testing:ReaderTests/AnnotationRepositoryTests`

### Completion Notes

- Добавлены lookup-методы `fetchHighlight`, `fetchTextNote`, `fetchPageNote` по связке `bookId + exchangeId`
- Legacy rows с `NULL exchange_id` корректно игнорируются и возвращают `nil`
- Repository regression tests остались зелёными, новые сценарии покрыты unit tests
...
- 2026-04-22: Реализован repository lookup по `bookId + exchangeId` и добавлены unit tests
